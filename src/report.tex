...
% --- Restructured and Shortened Section 1.2.1 (approx. 500 words) ---
...
\todo[inline, caption={}] {
    \begin{itemize}
        ...
        \item plot whiskers (or maybe in results?). acc this should be in results as it makes sense that is quantitative evaluation of results.
...
\begin{itemize}
    ...
    \item Next Sentence Prediction - In this task, the model is trained on a binary classification task predicting the relationship between sentence pairs. Given two sentences \texttt{A} and \texttt{B} either (1) sentence \texttt{B} is the actual sentence that follows \texttt{A} 50\% of the time or (2) sentence \texttt{B} is randomly selected from the corpus 50\% of the time \cite{devlin2019bert}. This task aimed to help BERT understand sentence relationships, though later models like RoBERTa and MPNet found it might not always be beneficial \cite{liu2019roberta, song2020mpnet}. % Added RoBERTa context from memory, optional
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{BERT1.png} % Assuming BERT1.png is the correct image file
    ...
    \label{fig:bert1_appendix} % Changed label to avoid conflict
\end{figure}
...
\begin{align}
    p(x) &= \prod_{i=1}^{T} p(x_i | X_{<i}) \label{eq:fwdprod_appendix} \\ % Relabeled
    p(x) &= \prod_{i=T}^{i} p(x_i | X_{>i}) \label{eq:bckprod_appendix} % Relabeled
\end{align}
...
The crucial link to PCA is that the columns of the matrix $V$ are precisely the principal component directions we seek. That is, the matrix $C$ in Equation~\ref{eq:pca_appendix}, containing the top $k$ principal components, corresponds to the first $k$ columns of $V$. % NOTE: Check if Equation reference needs updating
