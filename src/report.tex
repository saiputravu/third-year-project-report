...
I would also like to thank my family for their unwavering support during my university years.
...
In industry, production system uptime is strongly coupled with equipment maintenance. Maintenance, once seen as a `necessary evil', is now often viewed as a `profit contributor' for maintaining a competitive edge \cite{waeyenbergh2002framework, faccio2014industrial}. For research facilities like ISIS, maintenance directly impacts downtime and operational costs. Consequently, to minimise unexpected downtime and maximise efficiency, many industrial applications collect vast quantities of lifecycle data, potentially including process information, events, and alarms from various system components \cite{carvalho2019systematic}.
...
In the literature, various terms and categories of maintenance arise each with differing strategies \cite{susto2012predictive, mobley2002introduction, susto2016dealing}. Thus, while there exists some disagreement in nomenclature, we consider the four categories presented in (\citet{susto2012predictive}). The four maintenance policy categories are as follows, noting that each policy has unique benefits and drawbacks:
...
\begin{enumerate}
    ...
    \item{\textbf{Run-to-failure (R2F) maintenance} - Operate equipment until failure, then restore. This simple approach is typically costly due to unplanned downtime and potential need for complete component replacement.}
    \item{\textbf{Preventative maintenance (PvM)} - Scheduled maintenance performed at regular intervals to increase longevity or anticipate end-of-life. While preventing some failures, it can waste resources on healthy components, causing unnecessary downtime.}
    \item{\textbf{Condition-based maintenance (CBM)} - Maintenance triggered by continuously monitoring component health characteristics and metrics. This allows timely intervention when needed but hinders advance planning.}
    \item{\textbf{Predictive maintenance (PdM)} - Uses prediction tools (often statistical or machine learning models) to forecast failures and schedule maintenance proactively. PdM aims to optimise resource use, but its effectiveness depends critically on prediction accuracy; poor predictions lead to unnecessary costs or unexpected downtime.}
\end{enumerate}
...
The PdM strategy stands out, as accurate predictive models can optimise the trade-off between improving equipment condition, reducing failure rates, and minimising maintenance costs \cite{carvalho2019systematic}. It enables foresight for pre-emptive scheduling of large-scale maintenance. As pointed out in Section~\ref{sec:isisbg}, the ISIS facility aims to strike a balance between PvM, CBM, and PdM through periods of large-scale scheduled maintenance and extensive data collection. This balance is achieved through the careful coordination between cycle scheduling, day-to-day crew-based monitoring, and the FLD \cite{thomason2019isis}.
...
Sentence similarity (or document similarity) is the NLP task of quantifying similarity between texts. The increasing digitisation of language motivates this task \cite{raju2022sentence}. Use cases include detection of academic malpractice via plagiarism \cite{lukashenko2007computer, baba2017plagiarism}, and text summarisation \cite {aliguliyev2009new, kumar2020semantic, jo2017k}. 
Similarity can be assessed lexically (based on syntactic overlap) or semantically (based on meaning) \cite{raju2022sentence}. Semantic similarity, our focus, includes three main types:
...
\begin{itemize}
    ...
    %
    \item \textbf{String-based similarity} - Measures direct string similarity, accounting for character sequences and composition, using fine-grained (character-based), coarse-grained (term-based), or hybrid approaches \cite{yu2016string}.
    \item \textbf{Knowledge-based similarity} - Measures relatedness using semantic networks (knowledge graphs), such as the popular WordNet approach \cite{budanitsky2001semantic}.
    \item \textbf{Corpus-based similarity} - Uses models trained on large text corpora to estimate similarity between sentence pairs. Examples include statistical models (i.e. LSA \cite{landauer1998introduction}, SVD \cite{steinberger2005text}) and early word embedding models (i.e. Word2Vec \cite{bojanowski2017enriching}, GloVe \cite{pennington2014glove}, fastText \cite{mikolov2013efficient}). % --- Slightly fewer examples listed ---
\end{itemize}
...
Corpus-based methods typically require converting text into numerical vector representations. Computing this representation involves converting unstructured text into vectors. This usually includes applying an embedding model (word or sentence level), often preceded by pre-processing steps like stop-word removal, case normalisation, or tokenisation to standardise the input \cite{tabassum2020survey}. % --- Removed feature extraction link ---
...
This vector representation is known as an embedding, residing in an embedding space. The dimensionality of this space is the embedding dimension. This concept is important because embedding space characteristics influence the model's ability to capture syntactic and semantic meaning, as the space encodes this information. (\citet{bojanowski2017enriching}) shows, using Word2Vec, that different embedding dimensions produce different results. Furthermore, poorly constructed embedding spaces degrade prediction accuracy.
...
Therefore, semantic sentence similarity often relies directly on sentence embedding (or text embedding): the NLP task of learning effective high-dimensional vector representations. Text embedding is covered further in Section~\ref{sec:sentenceEmbedding}. The advent of the Transformer architecture \cite{vaswani2017attention} and large language models led to dominant DL approaches with high parameter counts \cite{cao2024recent} handling extremely large token sequences. While older word embedding models are often termed second-generation \cite{cao2024recent}, newer generations include:
...
\begin{itemize}
    ...
    \item \textbf{Third-generation} - Contextualised embeddings. These models (i.e. ELMo \cite{sarzynska2021detecting}, GPT \cite{radford2018improving}, BERT \cite{devlin2019bert}) dynamically encode context and are often referred to as language models.
    \item \textbf{Fourth-generation} - Universal text embeddings. The current state-of-the-art, aiming for unified models applicable to multiple downstream tasks. Examples include Gecko \cite{lee2024gecko}, Multilingual E5 \cite{wang2024multilingual}, Nomic \cite{nussbaum2024nomic}.
\end{itemize}
...
Modern (2nd-4th gen) text embedding models are frequently used in PdM for applications such as insight extraction and clustering intents from unstructured text data \cite{abijith2023large,usuga2022using}. Such datasets in industrial applications often come from operational or managerial logs documenting failures, resolutions, and comments. Advanced models enable semi- or fully automatic insight retrieval and auto-categorisation, facilitating intuitive understanding and potentially highlighting patterns in failure \cite{nota2022text}.
...
\begin{enumerate}
    \item \textbf{High-dimensional Graph Construction} - First, UMAP builds a weighted graph representation of the data in the original high-dimensional space. It identifies nearest neighbours for each data point and assigns edge weights based on the likelihood that points are closely connected on the underlying manifold. This step primarily focuses on capturing the local connectivity and structure of the data.
    \item \textbf{Low-dimensional Layout Optimisation} - Then, UMAP seeks a corresponding low-dimensional embedding where the graph structure is as similar in structure as possible to the high-dimensional graph. It iteratively adjusts the positions of points in the low-dimensional space to minimise the difference (often measured using cross-entropy, a measure of divergence between probability distributions \cite{zhang2018generalized}) between the connectivity probabilities in the high- and low-dimensional representations. This optimisation aims to keep points connected in the low-dimensional space if they were connected in the high-dimensional space, and separated if they were not.
\end{enumerate}
...
Density-Based Spatial Clustering of Applications with Noise (DBSCAN) is density-based clustering algorithm, unlike partitioning methods like k-Medoids \cite{ester1996density}.
Instead of partitioning data around centres, DBSCAN groups points in high-density regions, separated by sparser areas. Key characteristics are its ability to find arbitrarily shaped clusters and potentially identify noise, without requiring a predefined number of clusters \cite{ester1996density}. The core intuition is that points belonging to the same conceptual group tend to lie close to each other, forming denser regions than noise or outliers.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{Eps}}: Defines the maximum distance (neighbourhood radius) between two points to be considered neighbours. It essentially sets the radius for identifying neighbours; lower values require points to form dense regions.
    \item \textbf{\texttt{MinPts}}: Specifies the minimum number of points required within a point's Eps-neighbourhood for it to be considered a \textbf{core point} (an interior point of a dense region). This defines the threshold for density; higher values may result in lower clusters with more points classified as noise.
\end{itemize}
...
Key concepts include the \textbf{Eps-neighbourhood} (points within radius \texttt{Eps}), \textbf{core points} (points with at least \texttt{MinPts} neighbours within \texttt{Eps}), \textbf{border points} (non-core points reachable from a core point), and \textbf{noise points} (neither core nor border). A point $q$ is \textbf{density-reachable} from a core point $p$ if they are connected by a chain of core points within their respective Eps-neighbourhoods \cite{ester1996density}.

To understand the algorithm's operation, several core concepts are essential \cite{ester1996density}
\begin{itemize}
    \item \textbf{Eps-neighbourhood} - Points within radius of \texttt{Eps}.
    \item \textbf{Density-reachable} - Point with a chain of core points, with the greatest distance between any pairs of points being less than or equal to \texttt{Eps}.
\end{itemize}
...
Conceptually, the algorithm operates by selecting an arbitrary unvisited point. If it is identified as a core point, a new cluster is initiated. This cluster is then expanded to include all points density-reachable from it (both core and border points), effectively following chains of neighbouring core points. Points not reachable from any core point are designated as noise \cite{ester1996density}. Note that the assignment of border points reachable by multiple clusters can be implementation-dependent but typically is assigned by the first cluster to reach it \cite{ester1996density}.
...
In general, DBSCAN offers significant advantages: it can identify clusters of arbitrary shape, is inherently robust to noise, and does not require the number of clusters to be specified beforehand. However, it also has limitations: its performance is sensitive to the choice of \texttt{Eps} and \texttt{MinPts}, and it can struggle with clusters of significantly varying densities \cite{ester1996density, campello2013density}. With appropriate index structures, its average runtime complexity is typically $O(n\log n)$ for $n$ data points \cite{ester1996density}. Like many distance-based algorithms, its effectiveness can degrade in very high-dimensional spaces due to the `curse of dimensionality' (Section~\ref{sec:dimred}) \cite{hinneburg1999optimal, verleysen2005curse}.
...
Hierarchical Density-Based Spatial Clustering of Applications with Noise (HDBSCAN) \cite{campello2013density} advances upon DBSCAN by generating a hierarchical clustering structure. It addresses DBSCAN's reliance on a single density threshold ($\epsilon$), improving identification of varying density clusters by effectively exploring all density levels ($\lambda = \frac 1 \epsilon$, for all $\epsilon \in \big[0, \infty \big)$) to build its hierarchy \cite{campello2013density}.
...
Key contributions are the hierarchy construction algorithm (producing a dendrogram) and a novel `cluster stability' measure used to extract an optimal flat partition containing the most persistent clusters \cite{campello2013density}. Extraction optimises total stability. Advantageously, HDBSCAN mainly requires only the $m_{\text{pts}}$ parameter (equivalent to DBSCAN's \texttt{MinPts}), whose effect as a density smoothing factor is relatively well-understood \cite{campello2013density}.
...
Importantly, the following discussion assumes basic familiarity with graph theory; (\citet{harris2008combinatorics}) provides a primer. \\
...
\noindent Theoretically, HDBSCAN relates to DBSCAN by viewing clusters as connected components of core points within a graph, where edges connect mutually reachable core points (based on $\epsilon, m_{\text{pts}}$), above varying density levels \cite{campello2013density}.
...
It utilises concepts derived relative to $m_{\text{pts}}$ \cite{campello2013density}:
\begin{itemize}
    \item \textbf{Core distance ($d_{\text{core}}(x)$)}: Distance to the $m_{\text{pts}}$-th nearest neighbour, measuring local density (lower is denser).
    \item \textbf{Mutual reachability distance ($d_{\text{mreach}}(x_p, x_q)$)}: Defined as \[\max \{ d_{\text{core}}(x_p), d_{\text{core}}(x_q), d(x_p, x_q) \}\]This metric effectively increases distance between points in sparser regions.
\end{itemize}
...
The algorithm constructs the hierarchy conceptually by first computing core distances, then building a graph weighted by mutual reachability distance, finding its Minimum Spanning Tree (MST), and deriving the hierarchy (dendrogram) from the sorted MST edge weights. This captures cluster structures across all density levels \cite{campello2013density}. \\


\noindent To extract a meaningful flat partition, HDBSCAN introduces `cluster stability'. 
This quantifies how persistent a cluster $C_i$ is across the range of density levels ($\lambda = \frac 1 \epsilon$). Stability $S(C_i)$ is calculated for each cluster by summing, for each point $x_j$ belonging to $C_i$, the range of $\lambda$ values for which $x_j$ remains part of that specific cluster \cite{campello2013density}. More formally, using $\lambda_{\text{min}}(C_i)$ (threshold where density point where cluster appears) and $\lambda_{\text{max}}(x_j, C_i)$ (threshold where density point leaves cluster):
...
Equation~\ref{eq:hdbscan-stability} adapts the concept of `excess of mass' (i.e. the total density in a cluster after it is formed) \cite{muller1991excess} to a set of discrete data points and density levels derived from the hierarchy.
Clusters where points persist over a larger range of $\lambda$ values (i.e. are more `stable') will have higher stability scores.
...
Using this stability measure, HDBSCAN selects the optimal flat clustering by finding a set of disjoint clusters from the hierarchy that maximizes total stability. This is achieved efficiently by traversing a simplified cluster tree (pruned using a minimum cluster size, $m_{cluster\_size}$, typically set to $m_{pts}$), effectively performing optimal `cuts' at different hierarchy levels based on cluster persistence \cite{campello2013density}. \\

\noindent In summary, by building a full hierarchy and selecting clusters based on persistence across density levels, HDBSCAN has been shown to be more robust than DBSCAN and excels at identifying clusters of varying densities. Its primary disadvantages can be potential sensitivity to the chosen distance metric and the $m_{pts}$ parameter \cite{campello2013density}.
...
This project transforms unlabelled Operalog data into a categorised dataset through a five-step pipeline (Figure~\ref{fig:pipeline}): (1) pre-processing and normalising the \texttt{FaultDescription} text to reduce noise; (2) generating high-dimensional semantic embeddings to capture meaning numerically; (3) projecting embeddings to a lower dimension using UMAP while preserving (topological) structure in the original high-dimensional space; (4) clustering the low-dimensional data, with configurations optimised using internal validation heuristics to group similar descriptions automatically; and (5) inducing descriptive labels for the discovered clusters using NLP techniques. The analysis focuses solely on the \texttt{FaultDescription} field, as the \texttt{FaultRepair} field generally lacks sufficient informational content, with the majority either empty or less than 6 characters long. A diagram of the pipeline is shown in Figure~\ref{fig:pipeline}.
...
Furthermore, punctuation tokens are converted into one of two categories: end-of-sentence (\texttt{EOS}) tokens or regular punctuation (\texttt{PUNC}) tokens. The goal for this is to normalise punctuation to either signify sentence boundaries or collapse into one token. It is not trivial to see whether the \texttt{FaultDescription} text utilises punctuation to convey additional meaning. Thus, the step was made parametrisable via a flag which decides whether punctuation mapping is enabled. Similarly, text casing normalisation (converting all text to lower-case) follows the same procedure. 
